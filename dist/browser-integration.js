...
exports.BrowserIntegration = void 0;
exports.createProtectedBrowser = createProtectedBrowser;
const protection_1 = require("./protection");
const logger_1 = require("./logger");
class BrowserIntegration {
    constructor(browser, options = {}) {
        this.protectedPages = new Set();
        this.protectedFrames = new Set();
        this.protectedWorkers = new Set();
        this.cdpSessions = new Set();
        this.browser = browser;
        this.protection = new protection_1.FingerprintProtection(options);
                ...
                switch (target.type()) {
                    ...
                    case 'other':
                        ...
                        try {
                            ...
                            if (worker)
                                await this.protectWorker(worker);
                        }
                        catch {
                    ...
                    case 'background_page':
                        ...
                        if (bgPage)
                            await this.protectBackgroundPage(bgPage);
                        break;
                }
            }
            catch (error) {
    ...
    async protectFrame(frame) {
        if (this.protectedFrames.has(frame) || frame.isDetached())
            return;
    ...
    async protectWorker(worker) {
        if (this.protectedWorkers.has(worker))
            return;
    ...
    async protectBackgroundPage(page) {
    ...
    async protectOtherTarget(target) {
    ...
    async setupResponseInterception(page) {
        try {
    ...
    async protectExecutionContext(client, context) {
        try {
    ...
    async handleFrameNavigation(client, frame) {
        try {
    ...
    handleSecurityStateChange(event) {
    ...
    containsFingerprinting(message) {
        const patterns = [
            'canvas',
            'webgl',
            'audio',
    ...
    async createProtectedBrowser() {
        const protectedBrowser = this.browser;
        // Add enhanced methods
        protectedBrowser.newProtectedPage = async (options) => {
