...
export class RealWorldDetectorTester {
      ...
      for (let attempt = 1; attempt <= maxRetries; attempt++) {
        try {
          ...
          await page.goto('https://kaliiiiiiiiii.github.io/brotector/', { 
            waitUntil: waitUntilOptions[attempt - 1] as any,
            timeout 
          });
  ...
  async testSannysoft(): Promise<DetectorTestResult> {
    ...
    try {
      ...
      const result = await page.evaluate(() => {
        ...
        const botDetected = tests.some(t => 
          t?.name?.toLowerCase().includes('webdriver') && !t.passed
        );
  ...
  async runAllTests(): Promise<DetectorTestSuite> {
