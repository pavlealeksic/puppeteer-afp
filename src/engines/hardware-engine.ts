...
export interface GPUConfig {
  ...
  model: string;
  architecture: string;
  memory: number; // VRAM in MB
  computeUnits: number;
  baseClockSpeed: number; // MHz
  memoryClockSpeed: number; // MHz
  shaderCores: number;
  rtCores?: number;
  tensorCores?: number;
  apiSupport: {
...
export interface PowerConfig {
  ...
  dischargingRate: number; // mA per hour
...
export interface ThermalConfig {
  ...
  fanCurve: Array<{ temp: number; speed: number }>;
...
export class HardwareEmulator {
  ...
  private initializePerformanceCounters(): void {
  ...
  private getMemoryScript(): string {
    return `
  ...
  private getSensorScript(): string {
    return `
  ...
  private getThermalScript(): string {
    return `
  ...
  private getNetworkScript(): string {
    return `
  ...
  private getPerformanceScript(): string {
    return `
  ...
  private getLifecycleScript(): string {
    return `
      // Hardware Lifecycle Simulation
...
class PowerModel {
  constructor(private config: PowerConfig) {}
}
...
export const hardwareConfigs: Record<string, HardwareConfig> = {
  'macbook-pro-m2': {
    cpu: {
      architecture: 'arm64',
      vendor: 'Apple',
